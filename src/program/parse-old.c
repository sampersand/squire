...
static void parse_func_call(struct function_call *out) {
	assert(soul);

	struct expression *args[SQ_JOURNEY_MAX_ARGC];
	out->arg_count = 0;

	while (take().kind != SQ_TK_RPAREN) {
		if (out->arg_count > SQ_JOURNEY_MAX_ARGC)
		...
		if (!(args[out->arg_count++] = parse_expression()))
			die("invalid argument #%d found in function call", out->arg_count-1);
	...
	out->args = memdup(args, sizeof_array(struct expression *, arg_count));
...
static struct primary *parse_primary() {
	...
	switch (take().kind) {
	case SQ_TK_LPAREN:
		primary.kind = SQ_PS_PFN_CALL;
		primary.fncall = xmalloc(sizeof(struct function_call));
		primary.fncall->journey = primary_ptr;
		primary.fncall->kind = SQ_PS_FNCALL_JOURNEY;
